...
import models
from losses import CrossEntropyLabelSmooth, ArcFace
...
parser = argparse.ArgumentParser(description='Train image model with arcface loss')
# Datasets
...
parser.add_argument('--arcface-s', type=float, default=30.0, help="value s for arcface loss")
parser.add_argument('--arcface-m', type=float, default=0.50, help="value m for arcface loss")
parser.add_argument('--embedding-size', type=int, default=512, help="embedding size for input of arcface loss")
...
def main():
    ...
    transform_train = T.Compose([
        ...
        torchvision.transforms.RandomErasing(p=0.5, scale=(0.02, 0.4), ratio=(0.3, 3.33),
    ...
    model = models.init_model(name=args.arch, num_classes=dataset.num_train_pids, loss={'arcface'}, use_gpu=use_gpu)
    print("Model size: {:.5f}M".format(sum(p.numel() for p in model.parameters())/1000000.0))

    criterion_xent = CrossEntropyLabelSmooth(num_classes=dataset.num_train_pids, use_gpu=use_gpu)
    arcface = ArcFace(in_features=args.embedding_size, out_features=dataset.num_train_pids,
                      s=args.arcface_s, m=args.arcface_m, bias=False)
    if use_gpu: arcface = arcface.cuda()
    ...
    for epoch in range(start_epoch, args.max_epoch):
        ...
        train(epoch, model, criterion_xent, arcface, optimizer, trainloader, use_gpu)
...
def train(epoch, model, criterion_xent, arcface, optimizer, trainloader, use_gpu):
    losses = AverageMeter()
    ...
    for batch_idx, (imgs, pids, _) in enumerate(trainloader):
        ...
        thetas = arcface(embeddings, pids)
        loss = criterion_xent(thetas, pids)
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
